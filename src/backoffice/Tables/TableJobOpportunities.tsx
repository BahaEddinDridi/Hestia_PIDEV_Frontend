...
import { fetchJobsByRoleAndFutureDeadline } from "../api";
import OpportunitiesNotAvailable  from "../Tables/OpportunitiesNotAvailable";
...
const currentJobs = filteredJobs
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await fetchJobsByRoleAndFutureDeadline(); 
        setJobs(data);
      } catch (error) {
         ...
         {currentJobs.map((item, index) => (
              <tr key={index}>
                ...
                <td className="hidden lg:table-cell border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                  {item.jobs.map((job, jobIndex) => (
                    <p key={jobIndex} className="text-greenadd dark:text-greenadd py-2">
