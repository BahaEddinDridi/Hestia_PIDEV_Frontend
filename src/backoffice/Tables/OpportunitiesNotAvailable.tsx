...
import { fetchJobsByRoleAndDeadline } from "../api";
...
const OpportunitiesNotAvailable = () => {
  ...
  const filteredJobs = jobs
    .filter((jobArray) => jobArray.jobs.length > 0)
    .map((jobArray) => ({
      ...jobArray,
      jobs: jobArray.jobs.filter((job) =>
  ...
  //Pagination 
  const totalJobs = filteredJobs.reduce((acc, jobArray) => acc + jobArray.jobs.length, 0);
  const indexOfLastJob = currentPage * jobsPerPage;
  const indexOfFirstJob = indexOfLastJob - jobsPerPage;
  const currentJobs = filteredJobs
    .map((jobArray) => ({
      ...jobArray,
      jobs: jobArray.jobs.slice(indexOfFirstJob, indexOfLastJob),
    }))
    .filter((jobArray) => jobArray.jobs.length > 0);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await fetchJobsByRoleAndDeadline (); 
        setJobs(data);
      } catch (error) {
...
const handleDeleteJob = async (id: string, username: string) => {
  ...
  return (
    ...
    />
    <button className='px-3 py-2 mx-1 border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-red-800 focus:border-red-800' onClick={startVoiceSearch}>
         ...
         {currentJobs.map((item, index) => (
              <tr key={index}>
                ...
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                  {item.jobs.map((job, jobIndex) => (
                    <p key={jobIndex} className="text-black dark:text-white py-2">
                      {job.jobTitle}
                    </p>
                  ))}
                </td>
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                  {item.jobs.map((job, jobIndex) => (
                    <p key={jobIndex} className="text-esprit dark:text-esprit py-2">
                      {new Date(job.jobApplicationDeadline).toLocaleDateString('fr-FR')}
                    </p>
                  ))}
                </td>
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                ...
                {item.jobs.map((job, jobIndex) => (
                  <div className="flex items-center space-x-3.5 py-3">
                    <button onClick={() => setSelectedJob(job._id)} className="hover:text-primary">
                      <svg
            ...
            <div className="fixed inset-0 flex items-center justify-center">
             ...
             <div className="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5  "style={{ maxHeight: '80vh', overflowY: 'auto' }}>
                     ...
                     <dl>
                         <dt className="mb-2 font-semibold leading-none text-gray-200 dark:text-black">Job Description :</dt>
                         <dd className="mb-4 font-light text-gray-500 sm:mb-5 dark:text-gray-800">
                          {showFullDescription ? job.jobDescription : getDescriptionPreview(job.jobDescription)}
                          {showFullDescription && (
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-6 h-6">
                      ...
                      <p className='mb-4 font-light text-gray-500 sm:mb-5 dark:text-gray-800'>{job.jobPost}</p></div>
                      <div className="flex-1 ... mb-2 font-semibold leading-none text-gray-900 dark:text-black"> 
                      <div className="flex items-center">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-6 h-6">
                      ...
                      Education
                       ...
                       <p className='mb-4 font-light text-gray-500 sm:mb-5 dark:text-gray-800'>{job.jobRequiredEducation}</p></div>
                      <div className="flex-1 ... mb-2 font-semibold leading-none text-gray-900 dark:text-black">
                      <div className="flex items-center">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-6 h-6">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M4.26 10.147a60.438 60.438 0 0 0-.491 6.347A48.62 48.62 0 0 1 12 20.904a48.62 48.62 0 0 1 8.232-4.41 60.46 60.46 0 0 0-.491-6.347m-15.482 0a50.636 50.636 0 0 0-2.658-.813A59.906 59.906 0 0 1 12 3.493a59.903 59.903 0 0 1 10.399 5.84c-.896.248-1.783.52-2.658.814m-15.482 0A50.717 50.717 0 0 1 12 13.489a50.702 50.702 0 0 1 7.74-3.342M6.75 15a.75.75 0 1 0 0-1.5.75.75 0 0 0 0 1.5Zm0 0v-3.675A55.378 55.378 0 0 1 12 8.443m-7.007 11.55A5.981 5.981 0 0 0 6.75 15.75v-1.5" />
                     ...
                     Experience 
                         ...
                         <p className='mb-4 font-light text-gray-500 sm:mb-5 dark:text-gray-800'>{job.jobRequiredExperience}</p></div>
                      <div className="flex-1 ... mb-2 font-semibold leading-none text-gray-900 dark:text-black">
                      <div className="flex items-center">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-6 h-6">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M4.26 10.147a60.438 60.438 0 0 0-.491 6.347A48.62 48.62 0 0 1 12 20.904a48.62 48.62 0 0 1 8.232-4.41 60.46 60.46 0 0 0-.491-6.347m-15.482 0a50.636 50.636 0 0 0-2.658-.813A59.906 59.906 0 0 1 12 3.493a59.903 59.903 0 0 1 10.399 5.84c-.896.248-1.783.52-2.658.814m-15.482 0A50.717 50.717 0 0 1 12 13.489a50.702 50.702 0 0 1 7.74-3.342M6.75 15a.75.75 0 1 0 0-1.5.75.75 0 0 0 0 1.5Zm0 0v-3.675A55.378 55.378 0 0 1 12 8.443m-7.007 11.55A5.981 5.981 0 0 0 6.75 15.75v-1.5" />
                      ...
                      Skills 
                         ...
                         <p className='mb-4 font-light text-gray-500 sm:mb-5 dark:text-gray-800'>{job.jobRequiredSkills}</p></div>
                    </div>
                    <div className="flex ...">
                      <div className="flex-1 ... mb-2 font-semibold leading-none text-gray-900 dark:text-black">
                      <div className="flex items-center">
                      ...
                      Adress 
                        ...
                        <p className='mb-4 font-light text-gray-500 sm:mb-5 dark:text-gray-800'>{job.jobAdress}</p></div>
                      <div className="flex-1 ... mb-2 font-semibold leading-none text-gray-900 dark:text-black">
                      <div className="flex items-center">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-6 h-6">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M15 10.5a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
                      ...
                      Location 
                       ...
                       <p className='mb-4 font-light text-gray-500 sm:mb-5 dark:text-gray-800'>{job.jobLocation}</p></div>
                      <div className="flex-1 ... mb-2 font-semibold leading-none text-gray-900 dark:text-black">
                      <div className="flex items-center">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-6 h-6">
                      ...
                      Salary 
                       ...
                       <p className='mb-4 font-light text-gray-500 sm:mb-5 dark:text-gray-800'>{job.salary}$</p></div>
                      <div className="flex-1 ... mb-2 font-semibold leading-none text-gray-900 dark:text-black">
                      <div className="flex items-center">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-6 h-6">
                      ...
                      Contact Number 
                       ...
                       <p className='mb-4 font-light text-gray-500 sm:mb-5 dark:text-gray-800'>{job.contactNumber ? job.contactNumber.toString() : 'undifined'}</p></div>
                    </div>
                    <div className="flex ...">
                      <div className="flex-1 ... mb-2 font-semibold leading-none text-gray-900 dark:text-black">
                      <div className="flex items-center">
                      ...
                      <p className='mb-4 font-light text-gray-500 sm:mb-5 dark:text-gray-800'>{new Date(job.jobStartDate).toLocaleDateString('fr-FR')}</p></div>
                      <div className="flex-1 ... mb-2 font-semibold leading-none text-gray-900 dark:text-black">
                      <div className="flex items-center">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-6 h-6">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 0 1 2.25-2.25h13.5A2.25 2.25 0 0 1 21 7.5v11.25m-18 0A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75m-18 0v-7.5A2.25 2.25 0 0 1 5.25 9h13.5A2.25 2.25 0 0 1 21 11.25v7.5m-9-6h.008v.008H12v-.008ZM12 15h.008v.008H12V15Zm0 2.25h.008v.008H12v-.008ZM9.75 15h.008v.008H9.75V15Zm0 2.25h.008v.008H9.75v-.008ZM7.5 15h.008v.008H7.5V15Zm0 2.25h.008v.008H7.5v-.008Zm6.75-4.5h.008v.008h-.008v-.008Zm0 2.25h.008v.008h-.008V15Zm0 2.25h.008v.008h-.008v-.008Zm2.25-4.5h.008v.008H16.5v-.008Zm0 2.25h.008v.008H16.5V15Z" />
                      ...
                      <p className='mb-4 font-light text-gray-500 sm:mb-5 dark:text-gray-800'>{new Date(job.jobApplicationDeadline).toLocaleDateString('fr-FR')}</p></div>
                    </div>
                    <div className="max-w-lg mx-auto p-0">
                      <details className="open:bg-white dark:open:bg-slate-900  open:ring-black/5   rounded-lg" open>
                        ...
                        do you want to see other information about job?
                        </summary>
                        <div className=" text-sm leading-6 text-slate-600 dark:text-slate-400">
                          <p>{job.jobOtherInformation}</p>
                        </div>
                      </details>
                    </div>
            ...
            {showAlert && (
              <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 p-4 text-sm text-green-800 rounded-lg bg-green-50 dark:bg-gray-800 dark:text-green-400" role="alert">
                <span className="font-medium">Opportunity removal done successfully</span> 
                    ...
                    <button onClick={() => handleDeleteJob(job._id,item.username)} className="hover:text-red-900 text-red-500 ">
                      <svg
