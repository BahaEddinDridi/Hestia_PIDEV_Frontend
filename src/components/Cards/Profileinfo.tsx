          ...
          <div className=" w-full md:w-1/2 p-4">
            ...
            <ul className="grid gap-4 ">
              {currentUser.education && currentUser.education.length > 0 && currentUser.education.map((edu: any, index: number) => (
                <li key={index} className=" relative  p-4 border border-gray-200 dark:bg-white dark:text-black rounded-md  transition-transform transform hover:scale-105 cursor-pointer">
                  <div className="relative inline-block ">

                    <button id={`dropdownMenuIconButton-${edu._id}`} onClick={() => toggleDropdown(edu._id)} aria-haspopup="true" aria-expanded={isDropdownOpen[edu._id]} className="inline-flex ml-[360px] items-center w-5 h-5 p-2 text-sm font-medium text-center text-gray-900 bg-white rounded-lg hover:bg-gray-100 focus:ring-4 focus:outline-none dark:text-white focus:ring-gray-50 dark:bg-gray-800 dark:hover:bg-gray-700 dark:focus:ring-gray-600" type="button">
                    ...
                    {isDropdownOpen[edu._id] && (
                      <div id={`dropdownDots-${edu._id}`} className=" absolute right-0 z-1  bg-gray divide-y divide-gray-100 rounded-lg shadow-md  w-50  mt-1 lg:w-72 dark:bg-gray-700 dark:divide-gray-600" aria-labelledby={`dropdownMenuIconButton-${edu._id}`}>
